...
def write_inp(E11 = 150.0, E22 = 12.0, nu12 = 0.32, nu23 = 0.43, G12 = 4.69, t_ply = 0.125, XT = 2.724, XC = 1.690, YT = 0.064, YC = 0.2857, SL = 0.12, ST = 0.12, alpha = 0.0, K = 1.0e10, K_rig = 1e6, x_spring = 27.5, x_spring_fix = [], x_spring_load = [], K_ground = [], load = -10.0313, displacement = -4.0, rotation_offset = 0.0, StackSeq = [], init_inc = 1.0, min_inc = 1.0e-5, max_inc = 1.0, apply_load = True, fix_to_ground = True):
	...
	f.write('{}\n'.format(G23))
	# Damage initiation behaviour
	f.write('*DAMAGE INITIATION, CRITERION=HASHIN, ALPHA={}\n'.format(alpha)) # If no value of alpha is set, this defaults to zero as abaqus (and meng yi) does
	f.write('{}, {}, {}, {}, {}, {}\n'.format(XT, XC, YT, YC, SL, ST)) # Strength values
	...
	f.write('*NODE OUTPUT\n')
	f.write('U, RF\n')
	for i in range(6*len(StackSeq)//16): # 3 integration points per ply and layup is symmetric
		f.write('*ELEMENT OUTPUT, elset=m.All_elements\n')	
		for j in range(16):
			f.write('{}, '.format(16*i+j+1))
		f.write('\n')
		f.write('E, EE, S, DMICRT\n')
