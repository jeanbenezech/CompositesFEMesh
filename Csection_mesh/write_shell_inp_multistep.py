...
def write_inp(E11 = 115.6, E22 = 9.24, nu12 = 0.335, nu23 = 0.487, G12 = 4.826, t_ply = 0.196, K = 1.0e10, K_rig = 1e6, x_spring = 27.5, x_spring_fix = [], x_spring_load = [], K_ground = [], load = -10.0313, displacement = -4.0, rotation_offset = 0.0, StackSeq = [], init_inc = 1.0, min_inc = 1.0e-5, max_inc = 1.0, apply_load = True, fix_to_ground = True):
	...
	f.write('**\n')
	f.write('**---------- STEP ---------- \n')
	f.write('*STEP, NAME=Step-0, NLGEOM=YES, SOLVER=ITERATIVE, inc=100\n')
	f.write('*STATIC\n')
	f.write('1.0, 1.0, 1e-3, 1.0\n')
	# Continue with step up to 5.39kN
	f.write('*CLOAD \n')
	f.write('m.Load_Sup_RP, 3, -5.39\n')
	...
	# Csection
	if apply_load:
		f.write('*STEP, NAME=Step-0.5, NLGEOM=YES, SOLVER=ITERATIVE, inc=100\n')
		f.write('*STATIC\n')
		f.write('1.0, 1.0, 1e-3, 1.0\n')
		# Get to round number for tidyness
		f.write('*CLOAD \n')
		f.write('m.Load_Sup_RP, 3, -10.0\n')
		f.write('*OUTPUT, FIELD\n')
		f.write('*NODE OUTPUT\n')
		f.write('U, RF\n')
		f.write('*End Step\n')
